...
Definition cset_predType' : predType A.
Proof.
exact (mkPredType (fun t : convex_set A => (fun x => x \in CSet.car t))) ||
      exact (PredType (fun t : convex_set A => (fun x => x \in CSet.car t))).
Defined.
Canonical cset_predType := Eval hnf in cset_predType'.
...
(* NB(saikawa): hullI was removed from convex_choice.v as it was judged
   ...
   cases where inference of canonical structure does not work well for hull
   and a user needs to rewrite manually using such algebraic lemmas. *)
